...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Change theme and store selection
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  console.log('swither run')
  if (!codethemeSwitcher) return;

  // Retrieve and apply the saved theme if it exists
  const savedTheme = localStorage.getItem('code-theme');
  if (savedTheme) {
    codethemeSwitcher.value = savedTheme;
  ...
  setTimeout(applyBackgroundColor, 1000);  // Delay to ensure styles are applied

    localStorage.setItem('code-theme', selectedTheme);  // Save the selected theme
  });
  ...
  function applyTheme(theme) {
    ...
    // Check if it's the index.html page or not
    if (window.location.pathname.includes('index.html')) {
      pathToTheme = `highlight/styles/${theme}.css`; // For index.html, use an absolute path
    } else {
      pathToTheme = `../highlight/styles/${theme}.css`; // For other pages, use relative path
    }

    if (highlightCssLink) {
      highlightCssLink.href = pathToTheme;
    } else {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = pathToTheme;
